...
function panel(){
  ...
  if(tab==='color'){
    ...
    if(!visual)html+='<p class="sub">映像クリップを選択してください。</p>';
      ...
      html+='<p class="sub">選択クリップ：'+esc(c.name)+'</p><div class="button-grid">'+btn('clipColorReset','このクリップの色調整をリセット')+btn('clipTextureReset','このクリップの質感をリセット')+'</div><p class="sub">色調整はノイズ除去・色合わせ・ルックを含みます。質感は粒子・光の拡散・輪郭の柔らかさ・にじみ・走査線です。</p><details open data-panel-section="noise"><summary>1 · ノイズを抑える</summary>'+toggle('g.noiseOn','ノイズ除去を有効にする',g.noiseOn)+controls('g.',g,[['denoiseChroma','色ノイズ',0,1],['denoiseLuma','輝度ノイズ',0,1]])+'<p class="sub">初期値は両方0（処理なし）。輪郭を保つ軽量な空間処理です。</p></details>';
      ...
      html+='<details data-panel-section="clip-look"><summary>4 · クリップのルック</summary>'+toggle('g.lookOn','クリップルック ON',g.lookOn)+controls('g.',g,[['density','デンシティー',-1,2],['depth','中間色・暗部へ集中',0,1],['protect','ハイライト保護',0,1],['curveLow','カーブ：暗部（25%）',-.2,.2],['curveMid','カーブ：中間（50%）',-.2,.2],['curveHigh','カーブ：明部（75%）',-.2,.2],['shadowWarmth','暗部：青 ← → 暖色',-1,1],['shadowTint','暗部：ピンク ← → 緑',-1,1],['lightWarmth','明部：青 ← → 暖色',-1,1],['lightTint','明部：ピンク ← → 緑',-1,1],['fade','退色',0,.5]]);
      ...
      html+='</details><details data-panel-section="clip-texture"><summary>クリップの質感</summary>'+toggle('g.textureOn','クリップ質感 ON',g.textureOn)+controls('g.',g,[['grain','粒子',0,1],['bloom','光の拡散（ブルーム）',0,1],['softness','輪郭の柔らかさ',0,1],['bleed','色のにじみ（横方向）',0,1],['scanlines','走査線（横縞）',0,1]])+'</details>';
      html+='<details open data-panel-section="global-look"><summary>作品全体のルック・質感</summary><p class="sub">すべての映像に適用。字幕にはかかりません。</p><label>作品プリセット<select id="lookSelect"><option value="">選択してください</option>'+Object.keys(looks).map(k=>'<option>'+k+'</option>').join('')+'</select></label><p class="sub" id="lookDescription"></p>'+toggle('l.lookOn','全体ルック ON',look.lookOn)+controls('l.',look,[...tone,['density','全体デンシティー',-1,2],['curveLow','カーブ：暗部',-.2,.2],['curveMid','カーブ：中間',-.2,.2],['curveHigh','カーブ：明部',-.2,.2],['shadowWarmth','暗部：青 ← → 暖色',-1,1],['shadowTint','暗部：ピンク ← → 緑',-1,1],['lightWarmth','明部：青 ← → 暖色',-1,1],['lightTint','明部：ピンク ← → 緑',-1,1],['fade','退色',0,.5]])+toggle('l.textureOn','全体質感 ON',look.textureOn)+controls('l.',look,[['grain','粒子',0,1],['bloom','光の拡散（ブルーム）',0,1],['softness','輪郭の柔らかさ',0,1],['bleed','色のにじみ（横方向）',0,1],['scanlines','走査線（横縞）',0,1]])+'<p class="sub">光の拡散：明るい部分を周囲へ柔らかく広げます。色のにじみ：輪郭の色が横に広がります。走査線：画面に横縞を重ねます。0で効果なし、1で最大です。</p>'+btn('lookReset','全体ルック・質感だけをリセット','wide')+'</details>';
