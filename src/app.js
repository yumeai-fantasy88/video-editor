...
function panel(){
  ...
  if(tab==='color'){
    ...
    if(!visual)html+='<p class="sub">映像クリップを選択してください。</p>';
      ...
      html+='</details><details data-panel-section="clip-texture"><summary>クリップの質感</summary>'+toggle('g.textureOn','クリップ質感 ON',g.textureOn)+controls('g.',g,[['grain','粒子',0,1],['bloom','光の拡散（ブルーム）',0,1],['bleed','色のにじみ（横方向）',0,1],['scanlines','走査線（横縞）',0,1]])+'</details>';
      html+='<details open data-panel-section="global-look"><summary>作品全体のルック・質感</summary><p class="sub">すべての映像に適用。字幕にはかかりません。</p><label>作品プリセット<select id="lookSelect"><option value="">選択してください</option>'+Object.keys(looks).map(k=>'<option>'+k+'</option>').join('')+'</select></label><p class="sub" id="lookDescription"></p>'+toggle('l.lookOn','全体ルック ON',look.lookOn)+controls('l.',look,[...tone,['density','全体デンシティー',-1,2],['curveLow','カーブ：暗部',-.2,.2],['curveMid','カーブ：中間',-.2,.2],['curveHigh','カーブ：明部',-.2,.2],['fade','退色',0,.5]])+toggle('l.textureOn','全体質感 ON',look.textureOn)+controls('l.',look,[['grain','粒子',0,1],['bloom','光の拡散（ブルーム）',0,1],['bleed','色のにじみ（横方向）',0,1],['scanlines','走査線（横縞）',0,1]])+'<p class="sub">光の拡散：明るい部分を周囲へ柔らかく広げます。色のにじみ：輪郭の色が横に広がります。走査線：画面に横縞を重ねます。0で効果なし、1で最大です。</p>'+btn('lookReset','全体ルック・質感だけをリセット','wide')+'</details>';
